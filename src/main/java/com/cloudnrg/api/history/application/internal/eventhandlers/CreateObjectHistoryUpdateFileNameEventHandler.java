...
import com.cloudnrg.api.history.domain.model.events.CreateObjectHistoryUpdateFileNameEvent;
...
@Service
@AllArgsConstructor
public class CreateObjectHistoryUpdateFileNameEventHandler {
    private final ObjectHistoryRepository objectHistoryRepository;
    private final ExternalFileService externalFileService;

    @EventListener(CreateObjectHistoryUpdateFileNameEvent.class)
    public void on(CreateObjectHistoryUpdateFileNameEvent event) {
        var file = externalFileService.fetchFileById(event.getFileId());

        var user = file.getUser();

        var action = Action.UPDATE_FILE_NAME;
